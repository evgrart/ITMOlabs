...
export class MainComponent implements OnInit {
  ...
  currentHighlightZone: string | null = null;
    ...
    this.api.addPoint(this.point).subscribe({
      next: (res) => {
        const newPoint = {
            ...
            r: this.point.r,
            hit: res.inside,           
            smtVerified: res.smtVerified,
            proof: res.proof,
            formattedTime: new Date().toLocaleTimeString() 
        };

        this.history = [newPoint, ...this.history];
        this.messageService.add({severity:'success', summary:'Выстрел', detail: newPoint.hit ? 'Попадание!' : 'Промах'});
      },
      error: (err) => {
        if (err.status === 401 || err.status === 403) {
            this.auth.logout();
        } else {
            this.messageService.add({severity:'error', summary:'Ошибка', detail:'Сбой системы наведения (Проверьте Z3)'});
        }
      }
    });
  }
  ...
  onStepHover(step: any) {
    if (step.type === 'INEQUALITY_CHECK') {
        this.currentHighlightZone = step.expression; 
    }
  }

  onStepLeave() {
    this.currentHighlightZone = null;
  }
}