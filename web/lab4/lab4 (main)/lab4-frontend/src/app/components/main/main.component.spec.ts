...
export class MainComponent implements OnInit {
  ...
  submitPoint() {
    ...
    this.api.addPoint(this.point).subscribe({
      ...
      error: (err) => this.messageService.add({severity:'error', summary:'Ошибка', detail:'Сбой системы наведения'})
    });
  }
  ...
  isValid(): boolean {
    if (this.point.y < -3 || this.point.y > 3 || isNaN(this.point.y)) {
      this.messageService.add({severity:'error', summary:'Ошибка', detail:'Y должен быть от -3 до 3'});
      return false;
    }
    if (this.point.r <= 0) {
