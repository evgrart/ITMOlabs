...
export class GraphComponent implements OnChanges, AfterViewInit {
  @Input() r: number = 0;
  @Input() points: any[] = [];
  @Input() highlightZone: string | null = null;
  ...
  draw() {
    ...
    if (this.r > 0) {
        ...
        const baseColor = 'rgba(78, 111, 230, 0.5)';
        const highlightColor = 'rgba(255, 215, 0, 0.7)';

        this.ctx.fillStyle = (this.highlightZone && this.highlightZone.includes('Sector')) ? highlightColor : baseColor;
        this.ctx.beginPath();
        this.ctx.moveTo(cx, cy);
        this.ctx.arc(cx, cy, rPx, Math.PI, 1.5 * Math.PI, false);
        this.ctx.fill();
        
        this.ctx.fillStyle = (this.highlightZone && this.highlightZone.includes('Rectangle')) ? highlightColor : baseColor;
        this.ctx.beginPath();
        this.ctx.rect(cx - rPx, cy, rPx, rHalfPx);
        this.ctx.fill();

        this.ctx.fillStyle = (this.highlightZone && this.highlightZone.includes('Triangle')) ? highlightColor : baseColor;
        this.ctx.beginPath();
        this.ctx.moveTo(cx, cy);
